...
- **Multi-page, multi-project** — capture as many additional pages per project (beyond the homepage) as you need, and manage any number of separate projects from one dashboard.
...
[`CLAUDE.md`](CLAUDE.md) is a developer guide covering the architecture, data model, file structure, and conventions behind this project in more depth than this README. It's written so that an AI coding assistant (Claude Code, or similar tools that read a root-level guide file) can pick it up automatically and understand the codebase well enough to extend or modify it without searching through every file first — point yours at this repo and it should find it on its own.
